...
def run_test(label, role, system, action, setup_fn):
    print(f"\n{'='*50}")
    print(f"TEST: {label}")
    print(f"Role: {role} | System: {system} | Action: {action}")
    ctx = BehavioralContext(f"{role}_01", role, system)
    setup_fn(ctx)
    result = evaluate_event(role, system, action, ctx)
...
# ── Test 1: Normal student behavior ──────────────────
def test1_setup(ctx):
    ctx.set_baseline(0.05)
...
run_test(
    ...
    role="student",
    system="SIS",
    action="view own grades",
    setup_fn=test1_setup
)

# ── Test 2: Student attempting role violation ─────────
def test2_setup(ctx):
    ctx.set_baseline(0.05)
...
run_test(
    ...
    system="SIS",
    action="view own grades",
    setup_fn=test4_setup
)

# ── Test 5: High velocity ─────────────────────────────
def test5_setup(ctx):
    ctx.set_baseline(0.05)     # baseline: ~3 actions per minute
    ctx.files_downloaded = 0
    ctx.failed_attempts = 0
    # simulate 30 actions in rapid succession
    for _ in range(30):
        ctx.log_action("view own grades")
...
def test6_setup(ctx):
    ...
    ctx.files_downloaded = 0
    ctx.failed_attempts = 0
    ctx.concurrent_sessions = 3     # logged in from 3 devices
...
run_test(
    ...
    system="SIS",
    action="view/edit grades for assigned classes",
    setup_fn=test6_setup
)

# ── Test 7: Everything at once (worst case) ───────────
def test7_setup(ctx):
    ctx.set_baseline(0.05)
...
run_test(
    ...
    system="SIS",
    action="edit grades",
    setup_fn=test7_setup
)

# ── Test 8: IT accessing student records ──────────────
def test8_setup(ctx):
    ctx.set_baseline(0.05)
    ctx.files_downloaded = 0
    ctx.failed_attempts = 0
...
run_test(
    ...
    setup_fn=test8_setup
)

# ── Test 9: Normal IT behavior ────────────────────────
def test9_setup(ctx):
    ctx.set_baseline(0.05)
    ctx.files_downloaded = 0
    ctx.failed_attempts = 0
...
run_test(
    ...
    role="it",
    system="SIS",
    action="edit system configuration",
    setup_fn=test9_setup
)

# ── Test 10: Nurse accessing wrong records ────────────
...
run_test(
    ...
    role="nurse",
    system="SIS",
    action="access academic records",
    setup_fn=test10_setup
)